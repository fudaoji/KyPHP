...
class Upload {
    ...
    public function __construct() {
        ...
        $this->thumbPath = UPLOAD_PATH . 'thumb/';
        $this->reducePath = UPLOAD_PATH . 'reduce/';
    }
    ...
    public function uploadImg() {
        ...
        $info = $file->rule('md5')->validate(['ext' => 'jpg,png,gif,jpeg'])->move(UPLOAD_PATH);
        ...
        if ($info) {
            ...
            if ($pictureInfo = model('picture')->getOneByMap(['name' => $name, 'mpid'=>$this->mid])) {//若有重复，则返回图片
                $res = [
                    'code' => 0,
                    'data' => [
            ...
            $image_path = UPLOAD_PATH . $saveName;
            ...
            if ($array = explode('/', $saveName)) {
                if (mkdir($this->thumbPath . $array[0], 0777, true)) {
                    $thumb = $this->thumbPath . $saveName;
                    $image = Image::open($image_path);
                    $image->thumb(300, 300, Image::THUMB_SCALING)->save($thumb);
                    if (mkdir($this->reducePath . $array[0], 0777, true)) {
                        $reduce = $this->reducePath . $saveName;
                        $image->thumb(600, 600, Image::THUMB_SCALING)->save($reduce);
                    }
                }
            }
