        ...
        if (strpos($template, "@") === false) {
            ...
            switch ($count) {
                case 1:
                    ...
                    $template = ADDON_PATH . $this->addonName . '/view/' . $_theme . '/' . $this->addonController . '/' . $template . '.' . $suffix;
                    break;
                case 2:
                    if (!empty($tpls[0]) && $isSuffix == true) {
                        $template = ADDON_PATH . $this->addonName . '/view/' . $_theme . '/' . $template . '.' . $suffix;
                    } else {
                        if (!empty($this->theme)) {
                            $_theme = $_theme . '/' . strtolower($tpls[0]);
                        }
                        $template = ADDON_PATH . $this->addonName . '/view/' . $_theme . '/' . $tpls[1] . '.' . $suffix;
                    }
                    break;
            }
        } else {
            ...
            switch ($count) {
                case 2:
                    $template = ADDON_PATH . $this->addonName . '/view/' . $_theme . '/' . $this->addonController . '/' . ($isSuffix == true ? $template : $tpls[1]) . '.' . $suffix;
                    break;
                case 3:
                    $template = ADDON_PATH . $this->addonName . '/view/' . $_theme . '/' . strtolower($tpls[1]) . '/' . ($isSuffix == true ? $template : $tpls[2]) . '.' . $suffix;
                    break;
            }
        }
